...
const firebaseConfig = {
  ...
  appId: "1:424982794250:web:3fcb62f5339141280dcff3",
  measurementId: "G-18D36YHQPZ"
};
...
const COMMENT_COLLECTION = "harumaki_comments";
...
async function loadData() {
  const res = await fetch("./harumaki_gohan.json", { cache: "no-store" });
  if (!res.ok) throw new Error("harumaki_gohan.json 加载失败");
  DATA = await res.json();
}
  ...
  return `
    <div class="page">
      <section class="panel hero">
        <div class="badge">Harumaki Gohan TI</div>
        <h1 class="hero-title">${escapeHtml(DATA.meta?.title || "测测你是哪首春卷饭")}</h1>
        ...
        <div class="hero-meta">
          ...
          <div class="meta-card">
            <div class="meta-label">氛围底色</div>
            <div class="meta-value">${escapeHtml(DATA.meta?.tone || "梦境、夏天、玻璃感与未完成的心事")}</div>
          </div>
        </div>
    ...
    <div class="page">
      <section class="panel">
        ...
        <section class="comment-section">
          ...
          <div class="footer-note">评论会保存到 Firebase。若移动端网络环境特殊，可能依旧会出现加载超时。</div>
        </section>
      </section>
    </div>
  `;
...
async function addComment() {
  ...
  try {
    await withTimeout(
      addDoc(collection(db, COMMENT_COLLECTION), {
        name,
        msg,
...
async function loadComments() {
  ...
  try {
    const q = query(collection(db, COMMENT_COLLECTION), orderBy("time", "desc"));
...
function generateCard() {
  ...
  ctx.fillStyle = "#3f8e84";
  ctx.font = "bold 52px sans-serif";
  ctx.fillText("Harumaki Gohan TI", 110, 178);

  ctx.fillStyle = "#23406a";
  ctx.font = "bold 68px sans-serif";
  wrapText(ctx, result?.name || "", 110, 320, 660, 86);

  ctx.fillStyle = "#60708b";
  ctx.font = "34px sans-serif";
  wrapText(ctx, result?.subtitle || "", 110, 470, 660, 52);

  ctx.fillStyle = "#40556f";
  ctx.font = "30px sans-serif";
  wrapText(ctx, result?.lyrics || "", 110, 610, 640, 48);

  ctx.fillStyle = "#6c7d98";
  ctx.font = "24px sans-serif";
  ctx.fillText(`梦感 ${normalized.A}  孤独 ${normalized.B}  透明 ${normalized.C}  出走 ${normalized.D}`, 110, 1000);
}
